...
import { KeyboardLayout, NativeSelection, StoredLine } from './src/types';
import { editorReducer, initialEditorState, rangeOf } from './src/editor/reducer';
import { graphemeCount, inferCursorFromDiff } from './src/editor/unicode';
import { loadLines, makeLine } from './src/storage/storage';
import { useDebouncedSave } from './src/storage/useDebouncedSave';
import { searchLines } from './src/utils/search';
import { colors } from './src/constants/colors';
import { computeKeyWidth, hitSlopFor, KEY_HEIGHT, radius, spacing, fontSize } from './src/constants/dimensions';
import { KHMER_KEYBOARD_LAYERS } from './src/data/khmer';
import { ENGLISH_ROWS_LOWER, ENGLISH_PUNCTUATION_ROW } from './src/data/english';
import { NUMBER_KEYBOARD_LAYERS } from './src/data/numbers';
import { SYMBOL_KEYBOARD_LAYERS } from './src/data/symbols';
import { SIGIL_CATEGORIES } from './src/data/sigils';
...
export default function App() {
  ...
  // dropped; anything else is a real native-driven change. Replaces a
  // suppress-next-N-events counter, which could desync if native fired
  // events out of the order we expected.
  ...
  // CLR now always ends any active edit too — previously it only wiped
  // the text box, leaving editingId set, so the next Add silently saved
  // over the wrong line (see the original master audit, item 20/40).
  ...
  // Current keyboard rows per layer/shift state.
  const currentRows: string[][] = useMemo(() => {
    if (keyboardLayout === 'khmer') {
      return shiftOn ? KHMER_KEYBOARD_LAYERS.shift : KHMER_KEYBOARD_LAYERS.base;
    }
    if (keyboardLayout === 'english') {
      return [...ENGLISH_ROWS_LOWER, ENGLISH_PUNCTUATION_ROW];
    }
    if (keyboardLayout === 'numbers') {
      return [...NUMBER_KEYBOARD_LAYERS.arabic, ...NUMBER_KEYBOARD_LAYERS.khmer];
    }
